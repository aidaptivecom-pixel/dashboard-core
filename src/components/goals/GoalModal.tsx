...
export function GoalModal({ isOpen, onClose, onSave, goal, spaces }: GoalModalProps) {
    ...
    const handleSave = () => {
        ...
        const newGoal: Goal = {
            ...
            description,
            space: selectedSpace,
            spaceIcon: selectedSpaceData?.icon || "🎯",
            color: selectedColor,
            progress: 0,
            objectives,
    ...
    return (
        <AnimatePresence>
            ...
            <motion.div
                ...
                className="fixed top-[5%] left-1/2 -translate-x-1/2 w-full max-w-2xl max-h-[90vh] z-50 overflow-hidden"
                onClick={(e) => e.stopPropagation()}
            >
                <div className="mx-4 rounded-2xl border border-border bg-background shadow-2xl flex flex-col max-h-[90vh]">
                    {/* Header */}
                    <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                        <div className="flex items-center gap-3">
                            ...
                            >
                                <Target className="h-5 w-5" style={{ color: selectedColor }} />
                            </div>
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Define tu meta y sus objetivos
                                </p>
                            </div>
                        </div>
                        <button
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="¿Qué querés lograr con esta meta?"
                                    rows={2}
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-3">
                                <label className="text-sm font-medium flex items-center gap-2">
                                    <Flag className="h-4 w-4" style={{ color: selectedColor }} />
                                    Objetivos
                                </label>
                            ...
                            <div className="space-y-3">
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            <button
                                                ...
                                                className="p-1 rounded hover:bg-accent text-muted-foreground hover:text-coral transition-colors"
                                            >
                                        ...
                                        <div className="space-y-1 mb-2">
                                            {objective.tasks.map(task => (
                                                <div key={task.id} className="flex items-center gap-2 text-sm pl-2">
                                                    ...
                                                    <button
                                                        onClick={() => handleRemoveTask(objective.id, task.id)}
                                                        className="p-0.5 rounded hover:bg-accent text-muted-foreground hover:text-coral transition-colors opacity-0 hover:opacity-100"
                                                    >
                                                        <X className="h-3 w-3" />
                                                    </button>
                                                </div>
                                        ...
                                        <div className="flex gap-2">
                                            <input
                                                ...
                                                className="flex-1 px-3 py-1.5 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-1 focus:ring-primary/20"
                                                onKeyDown={(e) => {
                                                    if (e.key === "Enter") {
                                                        handleAddTask(objective.id, e.currentTarget.value);
                                ...
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="flex-1 px-4 py-3 rounded-xl border border-dashed border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                                        onKeyDown={(e) => {
                                            if (e.key === "Enter") {
                                                handleAddObjective();
                                            }
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-3 rounded-xl border border-dashed border-border hover:border-primary hover:bg-primary/5 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
